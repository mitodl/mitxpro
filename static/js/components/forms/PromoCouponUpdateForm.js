...
const selectProducts = (coupon, products) => {
  const productIds = coupon.eligibility.map(
            ...
            <label htmlFor="promo_coupon">
              ...
              <Field
                ...
                onChange={(e) => {
                    ...
                    if (selectedCoupon) {
                    const activationDate = moment(selectedCoupon.activation_date).toDate();
                    const expirationDate = moment(selectedCoupon.expiration_date).toDate();
                    const selectedProducts = selectProducts(selectedCoupon, products);
        ...
        <div className="flex">
          <Field
            ...
            onClick={(evt) => {
              ...
              const cached = values.productSelectionsByType?.[newProductType] || [];
          ...
          <Field
            ...
            onClick={(evt) => {
              ...
              const courseRun = values.productSelectionsByType?.[PRODUCT_TYPE_COURSERUN] || [];
              const program = values.productSelectionsByType?.[PRODUCT_TYPE_PROGRAM] || [];
          ...
          <Picky
            ...
            onChange={(value) => {
              ...
              if (values.product_type === "") {
                ...
                setFieldValue(`productSelectionsByType.${PRODUCT_TYPE_PROGRAM}`, programProducts);
                setFieldValue(`productSelectionsByType.${PRODUCT_TYPE_COURSERUN}`, courseRunProducts);
              } else {
                // Normal mode — cache only current type
                setFieldValue(`productSelectionsByType.${values.product_type}`, value);
              }
              console.log(
                `productSelectionsByType.${values.product_type}`,
                values.productSelectionsByType,
              );
            }}
            dropdownHeight={200}
