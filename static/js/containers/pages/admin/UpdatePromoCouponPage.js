...
type DispatchProps = {|
  updatePromoCoupon: (coupon: Object) => Promise<Response<CouponPaymentVersion>>,
|};
...
export class UpdatePromoCouponPage extends React.Component<Props, State> {
  ...
  onModalSubmit = async () => {
    const { updatePromoCoupon, promoCoupons, forceRequest } = this.props;
        ...
        />
          ...
          {isUpdated ? (
            ...
            <PromoCouponUpdateForm
              ...
              products={products?.filter(product => product.is_private === false)}
            />
          )}
        </div>
      </DocumentTitle>
