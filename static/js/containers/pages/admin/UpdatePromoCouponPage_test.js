...
import { makeCourseRunProduct, makeProgramProduct } from "../../../factories/ecommerce";
...
describe("UpdatePromoCouponPage", () => {
  ...
  it("filters private products from being passed to the form", async () => {
    ...
    assert.isFalse(form.props().products.some(product => product.is_private === true));
  });
  ...
  it("displays a success message on the page after update", async () => {
    ...
    assert.equal(successMessage.find(".message-text").text(), "Promo coupon successfully updated.");
  });
  ...
  it("displays an error message when update fails", async () => {
    ...
    assert.equal(errorMessage.find(".message-text").text(), "Failed to update promo coupon.");
  });
  ...
  it("updates promo coupon when onModalSubmit is called", async () => {
    ...
    assert.equal(inner.state().responseMsg, "Promo coupon successfully updated.");

  });

  it("maps product IDs correctly before updating", async () => {
    const testCouponData = {
      id: "coupon1",
      products: [{ id: "1" }, { id: "2" }],
    };
    ...
    const updatePromoCouponStub = helper.sandbox.stub().resolves({
      body: {
        message: "Promo coupon successfully updated.",
      },
    });
    ...
    // Verify that product_ids was added correctly to the coupon data
    const expectedData = {
      ...testCouponData,
      product_ids: ["1", "2"],
    };
    
    sinon.assert.calledWith(updatePromoCouponStub, sinon.match(value => {
      return value.product_ids && 
             value.product_ids.length === 2 && 
             value.product_ids.includes("1") && 
             value.product_ids.includes("2");
    }));
  });
