...
class EnrollmentChangeCommand(BaseCommand):
    ...
    def fetch_enrollment(user, command_options):
        ...
        if not enrollment:
            msg = f"Enrollment not found for: {enrolled_obj}"
            raise CommandError(msg)
        if not enrollment.active and not force:
            msg = (
                "The given enrollment is not active ({}).\nAdd the -f/--force flag if"
                " you want to change the status anyway.".format(enrollment.id)
            )
            raise CommandError(msg)
    ...
    def create_program_enrollment(
        ...
        enrollment_params = {"user": to_user, "program": to_program}
        enrollment_defaults = {
            "company": existing_enrollment.company,
            "order": existing_enrollment.order,
        }
    ...
    def create_run_enrollment(
        ...
        enrollment_params = {"user": to_user, "run": to_run}
        enrollment_defaults = {
            "company": existing_enrollment.company,
            "order": existing_enrollment.order,
        }
