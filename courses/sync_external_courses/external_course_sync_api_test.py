...
def test_deactivate_missing_course_runs(
    ...
    mock_now = (
        now_in_utc() + timedelta(1)
    )  # By default the course run is created with updated_on set to now therefore adding timedelta
    mocker.patch(
    ...
    assert (
        course_run.updated_on.replace(microsecond=0) == mock_now.replace(microsecond=0)
    ) == (not expected_is_live)
