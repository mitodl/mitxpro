...
def backfill_run_tags_with_valid_suffix(course_run_qset):
    for run in course_run_qset:
        run_tag_match = re.search(TEXT_ID_RUN_SUFFIX_PATTERN, run.courseware_id)
        if run_tag_match:
...
def backfill_course_run_run_tags(apps, schema_editor):
    ...
    # For all course runs with a "+R<number>" courseware id, use that suffix
    # as the run_tag value.
    all_course_runs = CourseRun.objects.all()
    backfill_run_tags_with_valid_suffix(all_course_runs)

    # For all course runs without a "+R<number>" courseware id, but have a start date and a coursweare id
    # suffix that is short enough, use that suffix as the run_tag value. Runs without start dates are
    # ignored because it's assumed that they are most likely meant for testing and aren't important enough
    # to need a valid run_tag.
