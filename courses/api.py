    ...
    for run in runs:
        try:
            enrollment, created = CourseRunEnrollment.all_objects.get_or_create(
                ...
                run=run,
                order=order,
                defaults={"company": company, "edx_enrolled": edx_request_success},
            )
    ...
    for program in programs:
        try:
            enrollment, created = ProgramEnrollment.all_objects.get_or_create(
                user=user, program=program, order=order, defaults={"company": company}
            )
            if not created and not enrollment.active:
                enrollment.reactivate_and_save()
        except:  # pylint: disable=bare-except  # noqa: E722
...
def deactivate_run_enrollment(
    run_enrollment, change_status, keep_failed_enrollments=False  # noqa: FBT002
):
    """
...
def deactivate_program_enrollment(
    ...
    run_enrollment_params = (
        {"order_id": program_enrollment.order_id}
...
def defer_enrollment(
    ...
    if not force and not from_enrollment.active:
        msg = (
            "Cannot defer from inactive enrollment (id: {}, run: {}, user: {}). Set"
            " force=True to defer anyway.".format(
    ...
    if from_enrollment.run == to_run:
        msg = f"Cannot defer to the same course run (run: {to_run.courseware_id})"
        raise ValidationError(msg)
    if not force and not to_run.is_not_beyond_enrollment:
        msg = (
            "Cannot defer to a course run that is outside of its enrollment period"  # noqa: E501, RUF100, UP032
            " (run: {}).".format(to_run.courseware_id)
        )
        raise ValidationError(msg)
    if not force and from_enrollment.run.course != to_run.course:
        msg = (
            "Cannot defer to a course run of a different course ('{}' -> '{}'). Set"
            " force=True to defer anyway.".format(
    ...
    except EdxEnrollmentCreateError:  # pylint: disable=try-except-raise  # noqa: TRY302
        raise
