...
def format_datetime_for_filename(
    datetime_object, include_time=False, include_ms=False  # noqa: FBT002
):  # noqa: FBT002, RUF100
    """
...
def dict_without_keys(d, *omitkeys):
    ...
    """  # noqa: D401
    return {key: d[key] for key in d if key not in omitkeys}
...
def filter_dict_by_key_set(dict_to_filter, key_set):
    ...
    return {key: dict_to_filter[key] for key in dict_to_filter if key in key_set}
...
class ValidateOnSaveMixin(models.Model):
    ...
    def save(
        self, force_insert=False, force_update=False, **kwargs  # noqa: FBT002
