...
# Importing here because we need to use methods from this model and replicating the functionality  # noqa: E501
# would make the migrations complex since it would include replication of some of the Wagtail's Page model.  # noqa: E501
...
def check_and_generate_associated_product(
    apps, schema_editor, external_courseware, courseware_run_id  # noqa: ARG001
):
...
def migrate_external_courses(apps, schema_editor):
    ...
    for external_course in external_courses:
        # It is possible that we might find a course with same readable Id, In this case let's just mark that  # noqa: E501
        # as external and not change other things to keep on safe side from overwriting data.  # noqa: E501
        ...
        # It's possible for a course to have multiple runs already created in the system, To be on safe side if we get  # noqa: E501
        # existing course runs let's just update the external URL in them to be on safe side  # noqa: E501
        ...
        if existing_course_runs.exists():
            ...
            # To be safe, Let's create products only if there was no existing course run and we created  # noqa: E501
            # the first one ever.
...
def migrate_external_programs(apps, schema_editor):
    ...
    for external_program in external_programs:
        # It is possible that we might find a program with same readable Id, In this case let's just mark that  # noqa: E501
        # as external and not change other things to be on safe side from overwriting data.  # noqa: E501
        ...
        # To be safe, Let's create product only if there was no existing program we created the first one ever  # noqa: E501
        if is_created:
            check_and_generate_associated_product(
...
def migrate_external_courseware(apps, schema_editor):
    """Migrate the existing external courseware pages to Courseware(Course, Program) Django models"""  # noqa: E501
    ...
    operations = [
        ...
        migrations.AlterField(
            ...
            field=models.CharField(
                blank=True,
                help_text=(
                    "The readable ID of the external course. Appears in URL, has to be"
                    " unique."
                ),
                max_length=64,
                null=True,
        ...
        migrations.AlterField(
            ...
            field=models.CharField(
                blank=True,
                help_text=(
                    "The readable ID of the external program. Appears in URL, has to be"
                    " unique."
                ),
                max_length=64,
                null=True,
        ...
        # Commenting this, The fields in this data migration are being removed now as cleanup.  # noqa: E501
