...
def fetch_user(filter_value, ignore_case=True):  # noqa: FBT002
    ...
    try:
        ...
        msg = "Could not find User with {}={} ({})".format(
            filter_field,
            filter_value,
            "case-insensitive" if ignore_case else "case-sensitive",
        )  # noqa: E501, RUF100
        raise User.DoesNotExist(msg) from e
...
def fetch_users(filter_values, ignore_case=True):  # noqa: FBT002
    ...
    if len(filter_values) > len(unique_filter_values):
        msg = "Duplicate values provided ({})".format(
    ...
    if user_qset.count() != len(filter_values):
        ...
        msg = "Could not find Users with these '{}' values ({}): {}".format(
            filter_field,
            "case-insensitive" if ignore_case else "case-sensitive",
...
def find_available_username(initial_username_base):
    ...
    """  # noqa: E501, D401
    # Keeps track of the number of characters that must be cut from the username to be less than  # noqa: E501
    ...
    # Any query for suffixed usernames could come up empty. The minimum suffix will be added to  # noqa: E501
    ...
    while letters_to_truncate < len(initial_username_base):
        ...
        # Find usernames that match the username base and have a numerical suffix, then find the max suffix  # noqa: E501
        ...
        if max_suffix is None:
            ...
            # If the next suffix adds a digit and causes the username to exceed the character limit,  # noqa: E501
            # keep searching.
            if len(candidate_username) <= USERNAME_MAX_LEN:
                return candidate_username
        # At this point, we know there are no suffixes left to add to this username base that was tried,  # noqa: E501
        # so we will need to remove letters from the end of that username base to make room for a longer  # noqa: E501
        # suffix.
