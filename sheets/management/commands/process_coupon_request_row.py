...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):  # pylint:disable=missing-docstring
        parser.add_argument("-r", "--row", type=int, help="Row number in the Sheet")
        parser.add_argument("-t", "--trans-id", type=str, help="Transaction ID")
        parser.add_argument(
            "-f",
            "--force",
            action="store_true",
    ...
    def handle(self, *args, **options):  # pylint:disable=missing-docstring
        ...
        for row_index, coupon_req_row in coupon_request_handler.parsed_row_iterator():
            ...
            if (
                options["trans_id"]
                and options["trans_id"] != coupon_req_row.transaction_id
            ):
                continue
        ...
        row_summary = "transaction id: {}, row: {}".format(
            matching_req_row.transaction_id, matching_row_index
        )
