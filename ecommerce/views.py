...
class PromoCouponView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        latest_coupon_type_subquery = Subquery(
            CouponPaymentVersion.objects.filter(
                payment=OuterRef('payment_id')
            ).order_by('-created_on')
        ...
        serializer = PromoCouponGetSerializer(promo_coupons, many=True, context={"is_private": False})
        ...
        try:
            with transaction.atomic():
                previous_version_data = {
                    ...
                    if field.name not in ("id", "activation_date", "expiration_date", "created_on", "updated_on")
                }
