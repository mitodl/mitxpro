...
class StripeCheckoutResultView(View):
    """
    Where Stripe returns the learner after checkout.

    Stripe sends no payment data here -- the order is fulfilled by the webhook,
    which may not have arrived yet. If it has, this redirects straight on to
    the receipt. If it hasn't, it renders a page that polls until it does.
    """

    def get(self, request, *args, **kwargs):  # noqa: ARG002
        """Send the learner on, or hold them on the interstitial."""
        checkout_session_id = request.GET.get("session_id", "")
        purchased = request.GET.get("purchased", "")
        receipt_url = (
            f"{reverse('user-dashboard')}?status=purchased&purchased={quote_plus(purchased)}"
            if purchased
            else reverse("user-dashboard")
        )
        ...
        if order is not None and order.status == Order.FULFILLED:
            return redirect(receipt_url)

        return render(
            request,
            "ecommerce/stripe_checkout_result.html",
            {
                "session_id": checkout_session_id,
                "receipt_url": receipt_url,
                "status_url": reverse("stripe-order-status"),
                "order_failed": order is not None and order.status == Order.FAILED,
            },
        )
